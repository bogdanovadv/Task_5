...
def start(message):
    ...
    if requests.post(url=host + '/users', data={'id':message.from_user.id}):
...
def cont(message):
    ...
    if message.text == 'Подписки по категориям':
        ...
        keyboard = telebot.types.ReplyKeyboardMarkup(True)
        news = requests.get(url=host + '/subscriptions/categories', params={'id':message.from_user.id}).json()
        for x in news:
        ...
        keyboard = telebot.types.ReplyKeyboardMarkup(True)
        news = requests.get(url=host + '/subscriptions/keywords', params={'id':message.from_user.id}).json()
        for x in news:
            if message.text.find("Удалить") == 0:
                x = "Удалить " + x
            keyboard.row(x)
        ...
        news = requests.get(url=host + '/news', params={'category':message.text}).json()
    ...
    elif message.text in requests.get(url=host + '/subscriptions/keywords', params={'id':message.from_user.id}).json():
        news = requests.get(url=host + '/news', params={'keyword':message.text}).json()
        if news['totalResults'] == 0:
            bot.send_message(message.chat.id, "Новостей нет", reply_markup=main())
        ...
        if message.text.split()[1] in requests.get(url=host + '/subscriptions/categories', params={'id':message.from_user.id}).json():
            if requests.delete(url=host + '/subscriptions/categories', data={'id':message.from_user.id, 'category':message.text.split()[1]}):
        ...
        if message.text.split()[1] in requests.get(url=host + '/subscriptions/keywords', params={'id':message.from_user.id}).json():
            if requests.delete(url=host + '/subscriptions/keywords', data={'id':message.from_user.id, 'keyword':message.text.split()[1]}):
                bot.send_message(message.chat.id, "Подписка удалена", reply_markup=main())
            else:
                bot.send_message(message.chat.id, 'Ошибка', reply_markup=main())
        ...
        keyboard = telebot.types.ReplyKeyboardMarkup(True)
        for x in category:
            if not x in requests.get(url=host + '/subscriptions/categories', params={'id':message.from_user.id}).json():
                keyboard.row("Добавить " + x)
        bot.send_message(message.chat.id, "Выберите подписку", reply_markup=keyboard)

    elif message.text.find("Добавить") == 0:
        if message.text.split()[1] in category:
            if requests.post(url=host + '/subscriptions/categories', data={'id':message.from_user.id, 'category':message.text.split()[1]}):
                bot.send_message(message.chat.id, "Подписка добавлена", reply_markup=main())
            else:
                bot.send_message(message.chat.id, 'Ошибка', reply_markup=main())
                
    elif flag:
        if not message.text in requests.get(url=host + '/subscriptions/categories', params={'id':message.from_user.id}).json():
            if requests.post(url=host + '/subscriptions/keywords', data={'id':message.from_user.id, 'keyword':message.text}):
